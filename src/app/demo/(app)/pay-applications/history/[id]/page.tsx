...
import { getDemoPayApp } from '@/lib/demo/data'
import { ArrowLeft, Download, FilePlus, AlertTriangle, FileQuestion } from 'lucide-react'

export default function DemoHistoryDetailPage({ params }: { params: { id: string } }) {
  const currentPayApp = getDemoPayApp(params.id)
  const totals = currentPayApp ? calculatePayAppTotals(currentPayApp) : null
          ...
          <div className="flex flex-wrap items-start justify-between gap-4">
            ...
            <div className="flex items-center gap-2.5 shrink-0">
              ...
              {currentPayApp.status === 'finalized' && (
                <button
                  type="button"
                  onClick={() => alert('Corrected drafts are disabled in the demo')}
